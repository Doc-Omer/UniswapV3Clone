...
// npx hardhat test test/SwapMultiHope.test.js
...
describe("SwapMultiHope", () => {
  ...
  before(async () => {
    ...
    const SwapMultiHope = await ethers.getContractFactory("SwapMultiHope");
    swapMultiHope = await SwapMultiHope.connect(accounts).deploy();
    ...
    console.log("==> swapMultiHope contract address: ", swapMultiHope.address);
    ...
    const singleSwap_JSON = require("../artifacts/contracts/SwapMultiHope.sol/SwapMultiHope.json");
    let singleSwap_ABI = new ethers.utils.Interface(singleSwap_JSON.abi); //providing interface of
    let singleSwap_contract = new ethers.Contract(swapMultiHope.address, singleSwap_ABI, accounts);
    console.log("==> swapMultiHope poolFees: ", await singleSwap_contract.poolFee());
    // console.log("==> swapMultiHope getEthBalance: ", await singleSwap_contract.getEthBalance(WETH9));
    // console.log("==> swapMultiHope getEthBalance2: ", await singleSwap_contract.getEthBalance2(WETH9));
  });



  it("swapExactInputMultihop", async () => {
    console.log("--swapExactInputMultihop start--");
    const amountIn = 10n * 10n**18n;


    // Deposit WETH
    await weth.deposit({ value: amountIn })
    await weth.approve(swapMultiHope.address, amountIn);
  
    // Swap
    const tx = await swapMultiHope.swapExactInputMultihop(amountIn);
    ...
    console.log("swapExactInputMultihop DAI balance", (await dai.balanceOf(accounts.address)).toString());
  });



  it("swapExactOutputMultihop", async () => {
    console.log("--swapExactOutputMultihop start--");
    const wethAmountInMax = 10n * 10n ** 18n;
    const daiAmountOut = 100n * 10n ** 18n;

    // Deposit WETH
    await weth.deposit({ value: wethAmountInMax });
    await weth.approve(swapMultiHope.address, wethAmountInMax);
    
    // Swap
    const tx = await swapMultiHope.swapExactOutputMultihop(daiAmountOut, wethAmountInMax);
    ...
    // check dai balance of signer
    console.log("swapExactOutputMultihop DAI balance", (await dai.balanceOf(accounts.address)).toString());
  })
});