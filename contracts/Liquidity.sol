...
contract LiquidityExample is IERC721Receiver{
    ...
    //@dev deposts[tokenId] => Deposit
    mapping(uint => Depost) public deposits;
    ...
    function onERC721Received(
        address operator, address, uint256 tokenId, bytes calldata
    ...
    function mintNewPosition() external returns(uint _tokenId, uint128 liquidity, uint amount0, uint amount1){
        ...
        //Providing liquidity in both assets means liquidity will be earning fees
        //and is in-range
        ...
        INonfungiblePositionManager.MintParams memory params = INonfungiblePositionManager.MintParams({
            ...
            //we are providing liquidity accross the whole range of the pool.
    ...
    function collectAllFees() external returns(uint amount0, uint amount1){
        //set amount0Max and amount1Max to uint256.max to collect all fees
        //alternatively can set to msg.sender and avoid another transaction in 'sendToOwner'
    ...
    function increaseLiquidityCurrentRange(uint amountAdd0, uint amountAdd1) external returns(uint liquidity, uint amount0, uint amount1) {
    ...
    function decreaseLiquidity(uint128 liquidity) external returns (uint amount0, uint amount1) external {
        INonfungiblePositionManager.DecreaseLiquidityParams memory params = INonfungiblePositionManager.DecreaseLiquidityParams.DecreaseLiquidityParams({
            tokenId: tokenId,
            liquidity: liquidity,
            amount0Min: 0,
            amount1Min: 0,
